...
func main() {
	...
	log := config.SetupLogger(cfg.Service.Env)
	url := fmt.Sprintf("nats://%s:%s", cfg.Nats.Host, cfg.Nats.Port)

	conn, err := nats.NewNatsConnection(cfg.Nats.ClusterId, cfg.Nats.SenderId, url)
	if err != nil {
	...
	for {
		...
		err := conn.Publish(cfg.Nats.Subject, []byte(OrderItem))
		if err != nil {
